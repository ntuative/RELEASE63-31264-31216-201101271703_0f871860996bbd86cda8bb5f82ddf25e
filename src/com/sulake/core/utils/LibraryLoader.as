package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1637:uint = 1;
      
      protected static const const_1635:uint = 2;
      
      protected static const const_1638:uint = 4;
      
      protected static const const_1636:uint = 3;
      
      protected static const name_2:uint = 7;
      
      public static const const_1576:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1086:Boolean = false;
      
      public static const const_1468:int = 2;
      
      private static var var_890:Array = new Array();
      
      private static var var_726:Array = new Array();
      ...
      protected var var_224:URLRequest;
      
      protected var var_644:XML;
      
      protected var var_529:Class;
      ...
      protected var var_2716:String = "";
      
      protected var var_2717:String = "";
      
      protected var var_1964:int;
      
      protected var var_1217:int;
      
      protected var var_1218:int;
      
      protected var var_2793:uint = 0;
      
      protected var var_2718:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_39.load(param1.var_224,param1.var_15);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1086)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_726[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_726.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_726.length < const_1468 && false)
            {
               _loc1_ = var_890.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_726.push(_loc1_);
                  _loc1_.var_39.load(_loc1_.var_224,_loc1_.var_15);
               }
            }
         }
      }
      ...
      {
         return !!this.var_224 ? this.var_224.url : null;
      }
      ...
      {
         return !!this.var_971 ? uint(this.var_1217 - this.var_1964) : uint(getTimer() - this.var_1964);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_39.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_224.url);
      ...
      {
         if(this.var_970 && !_disposed)
         {
            this.var_970 = false;
            if(!this.var_971 && this.var_224)
            {
               this.var_1217 = -1;
      ...
      {
         if(!this.var_971 && !_disposed && this.var_1218 > 0)
         {
            try
            {
               this.var_39.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_224,const_1576 - this.var_1218);
            if(this.var_2718)
            {
               this.var_224.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_224.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2718 = false;
            }
            --this.var_1218;
            this.var_431 = 0;
            this.var_39.load(this.var_224,this.var_15);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_366 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_366 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_366))
               {
                  this.var_1217 = getTimer();
         ...
         if(this.var_431 == LibraryLoader.const_1636)
         {
         ...
         if(this.var_431 == LibraryLoader.name_2)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_529 = this.getDefinition(this._name) as Class;
         if(this.var_529 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_224.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_529.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_366,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
