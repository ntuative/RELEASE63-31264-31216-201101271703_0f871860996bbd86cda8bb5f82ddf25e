package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1468:int = 4;
      ...
      {
         return this.var_415.length + this.var_263.length;
      }
      ...
      {
         if(!disposed && !this.isUrlInQueue(param1.url) && !this.method_7(param1.url))
         {
            if(param1.paused)
            {
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_263.length < const_1468 && this.var_415.length > 0)
            {
               _loc1_ = this.var_415.shift();
               this.var_263.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_415.indexOf(loader);
            if(index > -1)
            {
               this.var_415.splice(index,1);
            }
            index = this.var_263.indexOf(loader);
            if(index > -1)
            {
               this.var_263.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_415)
            {
               if(param2)
               {
      ...
      public function method_7(param1:String, param2:Boolean = true) : LibraryLoader
      {
         var _loc3_:* = null;
         if(!disposed)
         {
