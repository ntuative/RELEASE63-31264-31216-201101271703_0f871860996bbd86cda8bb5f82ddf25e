package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      public function show() : void
      {
         ...
         this.var_34.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_321));
         Logger.log("BEGINNING TO SHOW: " + this.var_321);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1460;
      }
      ...
      public function populate() : void
      {
         ...
         this.var_1012 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1012);
         this.var_1012.procedure = this.onSelectTemplate;
         this.var_911 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1387 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1386 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_34.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_34,WindowTracker.const_651,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_583 || this.var_516.text == "")
         {
            this.var_34.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_911.isSelected);
         this.var_34.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1436,_loc2_,this.var_516.text,"",""));
         if(this.var_1387.isSelected || this.var_1386.isSelected || this.var_911.isSelected)
         {
            this.var_34.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1387.isSelected,this.var_1386.isSelected,this.var_911.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1417);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1524);
      }
      
      private function onInputClick(param1:WindowEvent, param2:IWindow) : void
      {
         if(param1.type != WindowEvent.const_188)
         {
            return;
         }
         if(!this.var_583)
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_34.initMsg.roomMessageTemplates[this.var_1012.selection];
         if(_loc3_ != null)
         {
            this.var_583 = false;
