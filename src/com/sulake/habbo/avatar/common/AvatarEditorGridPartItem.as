package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_686:IFigurePartSet;
      ...
      private var var_561:Rectangle;
      
      private var var_1610:int = 0;
      
      private var var_381:IAvatarRenderManager;
      
      private var var_750:Boolean;
      
      private var var_1608:BitmapData;
      
      private const const_1928:int = 13421772;
      
      private const const_1929:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1610 = Math.max(this.var_1610,_loc6_.colorLayerIndex);
            }
         }
         this.var_381 = this.var_65.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_641 != null && !this.var_1609)
            {
               _loc3_ = this.var_641;
            }
            else
         ...
         if(this.isSelected)
         {
            this._window.color = this.const_1928;
         }
         else
         {
            this._window.color = this.const_1929;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_381.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_381.isFigureReady(_loc1_))
         {
            this.var_381.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_323[_loc6_] + "_" + FigureData.const_145;
               _loc4_ = this.var_381.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_561 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1608)
               {
                  ...
                  this.var_1608 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_561.width,this.var_561.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_561.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_561.y;
               if(this.var_1609 && _loc2_.colorLayerIndex > 0)
               {
