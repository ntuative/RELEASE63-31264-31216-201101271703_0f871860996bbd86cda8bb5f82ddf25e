package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_847:int = 3;
      
      private var var_383:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_383 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_383)
         {
            this.var_383.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_383.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_383)
         {
            this.var_383.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_383.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_847);
            if(this.var_1292.indexOf("?") > 0)
            {
               _loc2_ = this.var_1292 + "&retry=" + this.var_847;
            }
            else
            {
               _loc2_ = this.var_1292 + "?retry=" + this.var_847;
            ...
            this.var_383 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2231,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1058[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1058[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1642)
         {
            this.var_1057.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_495[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_495[_loc3_] = _loc6_;
            }
      ...
      {
         var _loc1_:* = null;
         while(this.var_695.length > 0 && this.var_694.length < this.const_1468)
         {
            _loc1_ = this.var_695[0];
            _loc1_.startDownloading();
            this.var_694.push(this.var_695.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_695.indexOf(param1) == -1 && this.var_694.indexOf(param1) == -1)
         {
