package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1337:String = "%";
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         this.var_575 = this.getKey("url_prefix");
         if(this.var_575 != null && this.var_575.length > 0)
         {
            this.setKey("client.fatal.error.url",this.var_575 + "/flash_client_error");
         }
         this.init();
      }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1551);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         for each(line in rows)
         {
            if(line.substr(0,1) != "#")
            {
               ...
               if(pair[0].length > 0 && pair[1].length > 0)
               {
                  ...
                  if(key == "configuration.readonly" && value == "AssetLoaderEventBroker")
                  {
                     readOnly = true;
                  }
         ...
         if(this.getKey("local.environment") == "AssetLoaderEventBroker")
         {
            this.setKey("url.prefix",this.getKey("url.prefix.local"));
         }
         this.var_575 = this.getKey("url.prefix");
         if(this.var_575.length > 0)
         {
            externalAccessDomain = this.var_575.replace("http://","");
      ...
      {
         var _loc2_:String = this.var_1094[param1] as String;
         return _loc2_ != null;
      }
      
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         var _loc4_:String = this.var_1094[param1] as String;
         if(_loc4_ == null)
         {
            _loc4_ = param2;
         }
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",this.var_575);
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1337);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1337,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
