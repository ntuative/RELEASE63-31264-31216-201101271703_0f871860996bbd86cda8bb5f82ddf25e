package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_229:IRoomObjectSprite;
      ...
      private var var_1245:ColorTransform;
      
      private var var_1512:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1245 = new ColorTransform();
         this.var_1512 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_92 = this.var_991[param1];
         this.var_812 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1511 >= 0)
         {
            this.var_229 = this._visualization.getSprite(this.var_1511);
            if(this.var_229 && this.var_229.asset)
            {
               if(this.var_229.width <= 1 || this.var_229.height <= 1)
               {
                  return;
               }
               if(this.var_81 == null)
               {
                  this.var_81 = this.var_229.asset.clone();
                  if(this.var_651.alphaMultiplier != 1)
                  {
                     this.var_538 = new BitmapData(this.var_81.width,this.var_81.height,true,4278190080);
                  }
               }
               this.var_2038 = -this.var_229.offsetX;
               this.var_2039 = -this.var_229.offsetY;
               this.var_229.asset = this.var_81;
            }
            if(this.var_81)
            {
            ...
            if(this.var_538)
            {
               this.var_538.fillRect(this.var_538.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_92.y * this.var_992;
         }
         return 0;
      }
      ...
      {
         if(!this.var_92 || !this.var_229)
         {
            return;
         }
         if(this.var_81 && this.var_229.asset != this.var_81)
         {
            this.var_229.asset = this.var_81;
         }
         if(this.var_812)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_812 && this.var_92.hasIgnited)
         {
            this.var_812 = true;
         }
         _loc8_ = this.var_1083 * this.var_992;
         this.var_92.update();
         if(this.var_812)
         {
            if(this.var_92.roomObjectSpriteId >= 0)
            ...
            {
               this.var_81.draw(this.var_538,this.var_1512,this.var_651,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_92.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2038 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_992;
               _loc4_ = this.var_2039 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_992;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1245.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_81.draw(_loc7_,this._translationMatrix,this.var_1245,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2037 = parseInt(param1.@size);
         this.var_1511 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1083 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_992 = this.var_2037 / 64;
         this.var_739 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_739 = Math.min(this.var_739,1);
         this.var_651.alphaMultiplier = this.var_739;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "AssetLoaderEventBroker" ? true : false;
               _loc21_ = [];
