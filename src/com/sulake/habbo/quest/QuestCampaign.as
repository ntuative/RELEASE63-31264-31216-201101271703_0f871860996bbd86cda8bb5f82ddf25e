package com.sulake.habbo.quest
{
   ...
   public class QuestCampaign implements IDisposable
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1162 = this._window.findChildByName("quest.container") as IWindowContainer;
         this.var_1406 = this._window.findChildByName("currency.container") as IWindowContainer;
         this.var_295 = this._window.findChildByName("quests") as IItemListWindow;
         var _loc1_:* = "icon_" + this.code + "_pixeltype";
         var _loc2_:IBitmapWrapperWindow = this.var_1406.findChildByName("currency.icon") as IBitmapWrapperWindow;
         this._controller.questEngine.setImageFromAsset(_loc2_,_loc1_,this.onPreviewImageReady);
         this.var_1406.findChildByName("currency.amount").caption = "${quest_" + this.var_1814 + ".currency}";
         var _loc3_:String = "quest_" + this.code;
         var _loc4_:* = _loc3_ + ".title";
         var _loc5_:* = _loc3_ + ".currency";
         this.setText("currency.amount",_loc5_);
         this.var_1161 = this._window.findChildByName("help.button") as IWindowContainer;
         if(this.var_1161)
         {
            this.var_1161.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.helpButtonEventProc);
            this.var_1161.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_OVER,this.helpButtonEventProc);
            this.var_1161.addEventListener(WindowMouseEvent.const_25,this.helpButtonEventProc);
         }
      }
      ...
      private function openHelp() : void
      {
         ...
         if(this.var_163)
         {
            this.var_1162.addChild(this.var_163.view);
         }
      }
      
      public function updatePurse() : void
      {
         var _loc1_:int = this._controller.questEngine.getActivityPointsForType(this.var_1657);
      ...
      private function onPreviewImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc3_ = this.var_1406.findChildByName("currency.icon") as IBitmapWrapperWindow;
      ...
      public function openCampaign(param1:Array) : void
      {
         ...
         {
            this.var_360.push(new Quest(_loc2_,this));
         }
         _loc3_ = null;
         for each(_loc4_ in this.var_360)
         {
            this.var_295.addListItem(_loc4_.view);
            if(!_loc3_ && (_loc4_.status == QuestStatusEnum.const_44 || _loc4_.status == QuestStatusEnum.const_498))
            {
               _loc3_ = _loc4_;
            }
         }
         ...
         else if(this.var_360.length > 0)
         {
            this.activateListItem(this.var_360[0]);
         }
      }
      ...
      private function showQuestData(param1:Quest) : void
      {
         ...
         this.var_296 = new QuestView(this,param1);
         this.var_1162.addChild(this.var_296.view);
         this.var_296.init();
      }
      ...
      {
         this.modifyQuestData(param1);
         this.var_752.push(new QuestCompleted(this,param1));
      }
      ...
      public function closeQuestCompleted(param1:QuestCompleted) : void
      {
         ...
         var _loc3_:* = null;
         var _loc4_:int = 0;
         for each(_loc3_ in this.var_752)
         {
            if(_loc3_ == param1)
            {
               _loc2_ = _loc3_;
         ...
         {
            _loc2_.dispose();
            _loc4_ = this.var_752.indexOf(_loc2_);
            if(_loc4_ >= 0)
            {
               this.var_752.splice(_loc4_,1);
            }
         }
      }
