package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_801 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_311 = [];
         this.var_531 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_801,this);
               if(_loc9_)
               {
                  this.var_531 = _loc9_.sellableBreeds;
                  this.var_311 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_531.length)
         {
            _loc10_ = this.var_531[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_801,_loc10_.id),this.getRaceLocalizationKey(this.var_801,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_801,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1164,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_311)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_441 = param1.index;
         if(this.var_441 < 0 || this.var_441 > this.var_311.length)
         {
            this.var_441 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_441 >= 0 && this.var_441 < this.var_311.length)
         {
            _loc7_ = this.var_311[this.var_441] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_531[this.var_1494] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_801,_loc3_.id,_loc2_,AvatarScaleType.const_53,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_303,AvatarAction.const_805);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_441 >= this.var_311.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_311[this.var_441] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_531[this.var_1494] as IBreed;
